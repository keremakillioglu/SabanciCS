...
int Robot:: BagContent () //this function returns the number of items in the robot's bag
{
	return bag;
}

bool Robot :: IsAlive() //this function checks whether robot is alive
{
	return !stalled ; 
}

int Robot:: GetXCoordinate () //this member funcion returns robot's xposition
{
	return xPos;
}

int Robot:: GetYCoordinate () //this member funcion returns robot's yposition
{
	return yPos;
}

void Robot :: TurnFace (Direction d1) // this function turns my robot to desired direction, with redrawing
{
	direction=d1;
	theRobotWindow-> Redraw(this);
}

Direction Robot :: GetDirection() //this member funcion returns my direction. since direction is private i need a member function to access this value
{
	return direction;
}

void Robot :: GhostTurn (Direction cengiz) // this function turns my robot to desired direction, without redrawing
{
	direction=cengiz;
}

void Robot :: InitializeBump() //this function initalizes my private member bump as 0
{
	bump =0 ;
}

void Robot :: SetBump() //this member function acts as a bump counter
{
	if (FacingWall()) //when robot faces wall at a single turn 
		bump=bump+1; //bump will increase 1 by 1, every time when robot crushes to wall
}


int Robot:: GetBump() // this member function returns my number of bumps
{
	return bump;
}
